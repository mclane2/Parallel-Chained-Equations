 ...
 * 
 * Description: Implements the equivalent of the "ENCE_impute_parallel" and "column_impute" R functions in C.
 * 
 ...
 * ence_impute_cycle() - A single chained equations cycle over each target station (with synchronous updates)
 * 
 * Version 2 - Added warm start optimization
 * Version 3 - Added covariance updates
 * Version 4 - Cleaned up comments, added references to thesis equations
 * 
 * Author: Marc Lane
 * Version: 4.0
 * Date: 2026-08-15
 ...
 *   cycle            - current cycle of chained equations
 ...
 * 
 */
static int column_impute(const double *df_old, double *target_out, int n, int p, int col, const int *missing_col, const int *fold_col, const double *lambdas, 
                         int n_lambda, const double *alphas,  int n_alpha, int nfolds, double thresh, int maxit, int cycle, double *lambda_sel, double *alpha_sel)
{
    const int p_cov = p - 1;   // number of covariate stations
    ...
    /* Count observed/missing days for this station */
    int n_obs = 0;
    int n_miss = 0;
    for (int i = 0; i < n; ++i) {
    ...
    if (n_miss == 0) return 0; // return early if there's no missing entries

    /* Allocating memory for all arrays */
    int *obs_days  = malloc((size_t)n_obs  * sizeof(int));            // observed indices of target station
    int *miss_days = malloc((size_t)n_miss * sizeof(int));            // missing indices of target station
    double *x_obs  = malloc((size_t)n_obs  * p_cov * sizeof(double)); // observed entries of covariate stations
    double *x_miss = malloc((size_t)n_miss * p_cov * sizeof(double)); // missing entries of covariate stations
    double *y_obs  = malloc((size_t)n_obs * sizeof(double));          // observed values of target station
    ...
    /* Malloc error check*/
    if (!obs_days || !miss_days || !x_obs || !x_miss || !y_obs || !beta || !C || !has_row) {
        free(obs_days); free(miss_days); free(x_obs); free(x_miss); free(y_obs); free(beta); free(C); free(has_row);
        return 1;
    }
    ...
    for (int k = 0; k < p_cov; ++k) { 
        ...
        int constant = 1;
        /* Fill this covariate station's values on the days the target station was observed into x_obs*/
        for (int io = 0; io < n_obs; ++io) {
            int day = obs_days[io];             // day index of the io-th observed day
            x_obs_col[io] = station_col[day];
            if (x_obs_col[io] != x_obs_col[0]) constant = 0; // flag if the array non-constant 
        }

        /* Error check for a constant (all the same values) covariate station across every observed day of the target (breaks standardise function) */
        if (constant == 1) {
            free(obs_days); free(miss_days); free(x_obs); free(x_miss); free(y_obs); free(beta); free(C); free(has_row);
            return 4;
        }
    ...
    int constant = 1;
    /* Construct the target vector y_obs from the days the target station was observed */
    ...
    /* Error check for constant (all the same values) target station across every observed day (breaks standardise function) */
    if (constant) {
        free(obs_days); free(miss_days); free(x_obs); free(x_miss); free(y_obs); free(beta); free(C); free(has_row);
        return 4;
    }
    ...
    if (lambda < 0.0) {
        ...
        if (!cvm || !fold_obs || !train_id || !val_id || !xt || !xv || !yt || !yv || !beta_path || !int_path ) {
            free(cvm); free(fold_obs); free(train_id); free(val_id); free(xt); free(xv); free(yt); free(yv);free(obs_days); free(miss_days); free(x_obs);
        ...
        /* Filling fold_obs with the fold ids for each value in obs_days */
        for (int io = 0; io < n_obs; ++io) fold_obs[io] = fold_col[obs_days[io]];
    ...
    for (int im = 0; im < n_miss; ++im) { 
        double yhat = intercept;
        for (int k = 0; k < p_cov; ++k) {
            yhat += x_miss[(size_t)k * n_miss + im] * beta[k]; // Equation 2.2.1
        }
        target_out[miss_days[im]] = yhat;
    }
    ...
    *alpha_sel  = alpha;
    ...
    /* This section only runs if the WRITE_BETAS flag is passed when compiling */
    #ifdef WRITE_BETAS
    /* Print Betas to file for analysis later */
    #pragma omp critical // make sure threads don't write over eachother
    {
        FILE *fp = fopen("betas.csv", "a");
        if (fp) {
            for (int k = 0; k < p_cov; ++k) {
                int station = (k < col) ? k : k + 1; // for each covariate station
                fprintf(fp, "%d,%d,%d,%.10g\n", cycle, col, station, beta[k]); // print the beta for each covariate station to the file
            }
            fclose(fp);
        }
    }    
    #else
    (void)cycle; // Ignore the cycle parameter if the WRITE_BETAS is not passed, since cycle is not used otherwise
    #endif
 ...
 *   7 - if diagnotic beta file fails to open (only for when WRITE_BETAS flag is passed)
 */
                      ...
                      const double *alphas,  int n_alpha, int nfolds, double thresh, int maxit, int nthreads, int cycle, double *lambda_sel, double *alpha_sel)
{
    ...
    #ifdef WRITE_BETAS
    /* Create file betas.csv on the first cycle only, later cycles append to it */
    if (cycle == 1) {
        FILE *fp = fopen("betas.csv", "w");
        if (!fp) return 7; // return 7 if failed to open file
        fprintf(fp, "cycle,target,covariate,beta\n"); // prints coefficients for each target station at every cycle
        fclose(fp);
    }
    #endif
    ...
    for (int col = 0; col < p; ++col) {
        ...
        status[col] = column_impute(df_old, df_new + (size_t)col * n, n, p, col, missing_idx + (size_t)col * n, folds + (size_t)col * n,
                      lambdas, n_lambda, alphas, n_alpha, nfolds, thresh, maxit, cycle, &lambda_sel[col], &alpha_sel[col]);
    }
