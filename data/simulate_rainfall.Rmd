...
The "simulated_daily_rainfall.csv" in the repo is 1 year of daily observations across 200 stations (p = 200,  n =365)
...
# z -> random errors
set.seed(222)
Z <- matrix(rnorm(no_stations * no_times),
            nrow = no_stations, ncol = no_times)

# changed from: L <- t(chol(t_cov)) %x% t(chol(s_cov))
L_s <- t(chol(s_cov))
L_t <- t(chol(t_cov))

# changed from: df$y <- trend + (L %*% z)
df$y <- as.vector(trend) + as.vector(L_s %*% Z %*% t(L_t))
