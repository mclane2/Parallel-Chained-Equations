...
|---|---|---|---|
| ENCE | R synchronous | 3.1× | — |
| ENCE | C / OpenMP | 11.4× | 3.6× |
| MICE DURR (m=1) | R synchronous | 2.6× | — |
| MICE DURR (m=1)| C / OpenMP |  7.0× | 2.7× |
...
This may require you install pandoc, which can be installed with:
```bash
...
For the simulated data (200 stations, 365 days) run on 6 cores on my laptop, serial ENCE takes approximately 230 seconds, synchronous R ENCE takes 50 seconds, and the C implementation of ENCE takes 12 seconds. Serial MICE DURR takes roughly 120 seconds, synchronous R MICE DURR takes 30 seconds and the C implementation takes roughly 11 seconds. Expect demo.Rmd altogether to take approximately 7.5 minutes to run.
